...
class BFPJ_Frontend_Fields {

    private $fields;
    private $field_order;

    public function __construct() {
        $this->fields = get_option( 'bfpj_fields', [] );
        $this->field_order = get_option( 'bfpj_field_order', [] );
    ...
    public function enqueue_scripts() {
        if ( is_checkout() || is_account_page() || is_wc_endpoint_url( 'edit-account' ) ) {
            ...
            wp_enqueue_script( 'bfpj-public-scripts', plugin_dir_url( __DIR__ ) . 'assets/js/bfpj-public-scripts.js', ['jquery', 'jquery-mask'], BFPJ_VERSION, true );
        }
    }
    ...
    public function add_fields_to_registration_form() {
        ...
        echo '<h2>Detalhes da Empresa</h2>';
        foreach ($custom_fields as $key => $settings) {
    ...
    public function validate_custom_fields( $errors, $username, $email ) {
        ...
        foreach ($active_fields as $key => $settings) {
            if (!empty($settings['required']) && $settings['required'] === 'yes' && empty($_POST[$key])) {
                $errors->add($key . '_error', sprintf(__('O campo <strong>%s</strong> é obrigatório.', 'bfpj'), $settings['label']));
            }
            if ( !empty($settings['validate_cnpj']) && $settings['validate_cnpj'] === 'yes' && !empty($_POST[$key]) ) {
                if ( !$this->is_cnpj_valid( $_POST[$key] ) ) {
                     $errors->add($key . '_error', sprintf(__('O <strong>%s</strong> informado não é válido.', 'bfpj'), $settings['label']));
                }
            }
        }
        return $errors;
    ...
    public function validate_custom_fields_checkout() {
        ...
        foreach ($active_fields as $key => $settings) {
            if ( !empty($settings['validate_cnpj']) && $settings['validate_cnpj'] === 'yes' && !empty($_POST[$key]) ) {
                if ( !$this->is_cnpj_valid( $_POST[$key] ) ) {
                     wc_add_notice( sprintf(__('O <strong>%s</strong> informado não é válido.', 'bfpj'), $settings['label']), 'error' );
                }
            }
        }
    }
    ...
    public function display_custom_fields_in_admin_order($order) {
        ...
        if (!empty($output)) {
            echo '<div><h4>' . __('Dados Adicionais PJ', 'bfpj') . '</h4>' . $output . '</div>';
        }
    }
